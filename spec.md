...
**Mô hình dữ liệu Firestore (đối chiếu trực tiếp với sheet "Quản lý tài chính 2026" thật — trang Ghi chép + Tổng hợp). Bản vẽ đầy đủ (sơ đồ use case, ERD, 17 màn hình mô phỏng) đã chốt tại [`docs/design.html`](docs/design.html) — phần dưới đây là bản tóm tắt kỹ thuật khớp với file đó, mọi thay đổi schema về sau phải cập nhật đồng thời cả hai nơi.**
...
| `cho_di` | **CĐ** (viết tắt "Cho đi") | EXPENSE | không | **có, 3 bước: CCB/ĐCB/ĐG** |
| `dang_hien` | **DH** (viết tắt "Dâng hiến") | EXPENSE | không | **có, 3 bước: CCB/ĐCB/ĐD** |
...
| `tiet_kiem` | Tiết kiệm | TRANSFER | không | không — dùng chung cho nạp/rút/chuyển đổi loại tài sản (`SavingsAssetType` bất kỳ, không riêng "ngân hàng"), phân biệt bằng `transferKind` |

**`CĐ`/`DH` viết tắt để bảo mật** — "Cho đi"/"Dâng hiến" là 2 hạng mục hơi nhạy cảm (liên quan niềm tin cá nhân), nên **tên hiển thị mặc định đổi thành mã viết tắt** "CĐ"/"DH" — ai nhìn lướt qua màn hình cũng không đoán ra ngay đây là gì. `id` nội bộ (`cho_di`/`dang_hien`) giữ nguyên, chỉ đổi `name`; đúng nguyên tắc "category là dữ liệu" — gia đình có thể đổi lại thành tên đầy đủ hoặc mã khác bất cứ lúc nào qua màn "Danh mục — Chỉnh sửa", không hardcode. 3 bước trạng thái của cả 2 danh mục cũng viết tắt: **CCB** = Chưa chuẩn bị, **ĐCB** = Đã chuẩn bị, **ĐD** = Đã dâng (chỉ DH), **ĐG** = Đã gửi (chỉ CĐ).
...
**Mẫu "tiền hộ người khác" (thu nhập có phần phải trả lại) — dùng Category + Status có sẵn, không cần cơ chế mới:** ví dụ thu 2.000.000đ doanh thu nhưng phải trả 1.000.000đ chi phí vận hành (thuê ngoài, không phải thành viên gia đình) — **không nhập số âm vào Thu nhập** (vi phạm `amountMinor` luôn dương, xem Invariant 12 ở `docs/financial-core-v2.md`). Thay vào đó ghi **2 giao dịch riêng**: 1 `INCOME` "Doanh thu" +2.000.000đ, và 1 `EXPENSE` dưới category tự tạo (vd "Chi phí vận hành") có `statuses: ["Chưa gửi", "Đã gửi"]` −1.000.000đ — đúng cơ chế trạng thái đã có sẵn cho danh mục có theo dõi tiến độ, để biết còn nợ ai bao nhiêu chưa trả. Số dư ròng vẫn tăng đúng 1.000.000đ; `Tổng thu`/`Tổng chi` hiện đúng cả 2 chiều tiền thay vì gộp tắt thành 1 số net, minh bạch hơn khi xem lại báo cáo.

**"Thu nhập ròng" — xem nhanh phần còn lại sau khi trừ khoản phải trả, không cần tự cộng trừ 2 category bằng tay.** Category `type=INCOME` có thêm field tuỳ chọn **`linkedExpenseCategoryId`**, trỏ tới 1 category `type=EXPENSE` khác (vd "Doanh thu" liên kết "Chi phí vận hành"). Khi đã liên kết, màn Danh mục/Tổng hợp tự tính và hiện thêm dòng **"Thu nhập ròng" = Doanh thu − Chi phí vận hành = Tổng thu category Thu − Tổng chi category Chi liên kết (cộng cả trạng thái Chưa gửi lẫn Đã gửi, vì tiền đã bị trừ khỏi số dư ngay lúc ghi, status chỉ là nhãn tiến độ)**. Đây **chỉ là số hiển thị thêm** — không đổi cách tính `Tổng thu`/`Tổng chi`/`Tổng tài sản` ở mục công thức tài chính, và **không gộp 2 giao dịch thành 1** (vẫn ghi Thu/Chi riêng như trên) — field này thuần để nối 2 category lại với nhau cho mục đích báo cáo, phần lớn category Thu khác (Lương, Thu nhập khác...) để trống. **Hiện ở 3 chỗ**: dòng phụ "Ròng X đ" ngay dưới tên category trong màn "Danh mục — Danh sách" (06), preview trực tiếp ngay khi chọn liên kết trong màn "Danh mục — Chỉnh sửa" (07, cập nhật theo dữ liệu hiện có mỗi khi đổi lựa chọn), và dòng chữ xanh trong màn "Trạng thái — Chi tiết" (13) khi xem category Chi đã liên kết. Xem field đầy đủ + test case ở `docs/financial-core-v2.md` mục 11, 17, 24 (Test 18).
...
**Phase 3, 6, 8-13, 15-17 đã code xong theo đúng Financial Core V2** (viết trong môi trường không có Flutter/Dart SDK để tự `flutter analyze`/`flutter test` — **bắt buộc chạy 2 lệnh đó + `dart run build_runner build --delete-conflicting-outputs`** trên máy có Flutter trước khi coi là DONE thật sự, xem `README.md`/báo cáo cuối phiên). Phase 7 và 14 (onboarding Local + `syncMode`) dời sang phiên sau — không đụng Financial Engine, tách riêng an toàn.
...
3. ✅ **CI cơ bản trên GitHub Actions** — Code: `.github/workflows/ci.yml` chạy `flutter analyze` + `flutter test` mỗi lần push/PR vào `master`. Chạy: push thử 1 commit. Test: Action chạy xanh.
...
5. ✅ **Domain entities & mock repository** — đã thay hẳn bằng entity/repository V2 thật ở phase 6 bên dưới, không còn mock.
6. ✅ **`LocalTransactionRepository` (lưu cục bộ trên máy)** — Code: `Transaction` V2 đầy đủ field (`type/source/destination/reversal...`), `AppDatabase` (Drift) schema mới (`TransactionRows/CategoryRows/StatusRows/FundRows`), `LocalTransactionRepository` implement `addTransaction`/`reverseTransaction`/`correctTransactionAmount`/`updateTransactionStatus` (không còn `updateTransaction`/`deleteTransaction` tự do). Test: `test/domain/financial_engine_test.dart` (Test 6/7/8/10 + Invariant 7).
7. ⏳ (dời phiên sau) **Luồng khởi động Local, không bắt đăng nhập** — 3 màn Chào mừng/Đăng nhập-Bỏ qua/Chọn Cá nhân-Gia đình chưa dựng — app hiện vào thẳng `AppShell`.
8. ✅ **Nối sheet Thêm giao dịch ghi vào Local** — Code: `add_transaction_sheet.dart` viết lại hoàn toàn theo màn 09 (Thu/Chi/Chuyển, danh mục đọc động từ `CategoryRepository`) gọi `LocalTransactionRepository` thật. Có chọn ngày giao dịch (`transactionDate`, mặc định hôm nay, tách khỏi `createdAt`) — không bắt buộc ghi đúng ngày tạo.
9. ✅ **Danh sách giao dịch theo ngày + chuyển xem tháng khác** — Code: `transaction_list_screen.dart` (tab "Giao dịch" mới trong bottom-nav).
10. ✅ **Sửa/Xoá giao dịch qua reversal ledger** — Code: `financial_engine.dart` (`buildReversal`/`buildCorrection`) + `LocalTransactionRepository.updateTransaction`/`reverseTransaction`. Màn `transaction_detail_screen.dart` sửa được toàn bộ (hạng mục trong cùng loại, số tiền, ghi chú, người tiêu, ngày, trạng thái) — `amountMinor`/người tiêu đổi thì tự động đi qua reversal ledger, các field còn lại update thẳng (mục 21, cập nhật mở rộng phạm vi so với bản đầu).
11. ✅ **Đổi trạng thái cho giao dịch đã ghi** — Code: gộp vào `TransactionRepository.updateTransaction` (update thẳng field `statusId`, không qua reversal vì không ảnh hưởng balance) + UI đổi trạng thái trong `transaction_detail_screen.dart`.
12. ✅ **Quản lý Danh mục & Trạng thái trên Local** — Code: `LocalCategoryRepository`/`LocalStatusRepository` + `category_list_screen.dart`/`category_edit_screen.dart` (tab "Danh mục" mới), gồm cả `linkedExpenseCategoryId` picker.
13. ✅ **Validate input khi ghi thật** — Code: `addTransaction` ném `InsufficientBalanceException` nếu âm pool; nút Lưu ở màn Thêm giao dịch khoá khi chưa đủ dữ liệu (`_buildTransaction` trả null).
14. ⏳ (dời phiên sau) **`syncMode: "local" | "cloud"`** — entity `SyncMode` đã có sẵn từ trước nhưng chưa gắn field lưu cục bộ nào (gộp cùng phase 7).
15. ✅ **`LocalFundRepository`, tạo được nhiều quỹ** — Code: bỏ hẳn `FundEntry` (F-11), `Fund` chỉ còn identity, số dư tính động qua `computeFundBalance`. **Cùng đợt:** tổng quát hoá Tiết kiệm theo mẫu tương tự — `SavingsAssetType` (dữ liệu tự tạo, không còn cố định "Hiện tại"/"Ngân hàng"), `PoolKind.memberSavingsAsset`, màn `savings_screen.dart` (Cài đặt → Tiết kiệm). Xem `docs/financial-core-v2.md` mục 9.
16. ✅ **Màn hình Danh sách Quỹ + tạo quỹ mới trên Local** — Code: `fund_list_screen.dart`. Màn Tổng hợp cũng bổ sung `computeMemberOutflowBreakdown`/`computeMemberIncomeTotal` (gộp Chi + Chuyển-là-nguồn theo từng thành viên, tách riêng Thu nhập) để khớp đúng cách sheet gốc liệt kê Đầu tư/Tự thưởng/CĐ/DH/Tiết kiệm/Chuyển tiền thành viên chung 1 bảng.
17. ✅ **Chi tiêu nguồn là Quỹ (`source = FUND`) + chặn quỹ âm** — Code: "Nguồn tiền" fund-pick trong `add_transaction_sheet.dart`, khoá lựa chọn nếu vượt số dư quỹ (client) + `InsufficientBalanceException` (server/repository).
18. **Mốc kiểm tra: dùng đầy đủ ghi chép + Danh mục/Trạng thái tự quản lý + Quỹ liên tục nhiều ngày, hoàn toàn không cần Firebase** — Chạy: dùng thử thật vài ngày sau khi `flutter analyze`/`flutter test` xanh. Test: không phát sinh lỗi nào đòi hỏi mạng/Firebase cho việc ghi chép cá nhân.
