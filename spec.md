...
8. ✅ **Nối sheet Thêm giao dịch ghi vào Local** — Code: `add_transaction_sheet.dart` viết lại hoàn toàn theo màn 09 (Thu/Chi/Chuyển, danh mục đọc động từ `CategoryRepository`) gọi `LocalTransactionRepository` thật.
...
10. ✅ **Sửa/Xoá giao dịch qua reversal ledger** — Code: `financial_engine.dart` (`buildReversal`/`buildCorrection`) + `LocalTransactionRepository.reverseTransaction`/`correctTransactionAmount`, màn `transaction_detail_screen.dart` chỉ cho sửa `amountMinor`/`statusId` đúng quyết định mục 21.
11. ✅ **Đổi trạng thái cho giao dịch đã ghi** — Code: `updateTransactionStatus` (update thẳng field, không qua reversal) + UI đổi trạng thái trong `transaction_detail_screen.dart`.
...
15. ✅ **`LocalFundRepository`, tạo được nhiều quỹ** — Code: bỏ hẳn `FundEntry` (F-11), `Fund` chỉ còn identity, số dư tính động qua `computeFundBalance`.
